...
-- --- Short Human-Readable IDs (Orders) ---------------------------------------
...
DROP TRIGGER IF EXISTS trg_job_short_id ON jobs;
CREATE TRIGGER trg_job_short_id BEFORE INSERT ON jobs
  FOR EACH ROW EXECUTE FUNCTION generate_job_short_id();

-- Update existing jobs
UPDATE jobs SET short_id = upper(substr(md5(job_id::text), 1, 8)) WHERE short_id IS NULL;
