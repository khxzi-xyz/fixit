      ...
      <div className="bg-primary text-primary-foreground border-b border-border px-4 pt-10 pb-14">
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 border border-white/20 rounded-full text-white text-xs font-bold hover:bg-white/20"
          >
            <Plus className="w-3.5 h-3.5" /> Add New
          </button>
        </div>
        <h1 className="text-2xl font-black text-white mt-3">My Addresses</h1>
        <p className="text-primary-foreground/70 text-sm mt-1">{addresses.length} saved location{addresses.length !== 1 ? "s" : ""}</p>
      </div>
      ...
      <div className="px-4 -mt-6 pb-10 space-y-4">
            ...
            {addresses.map((a: any, i: number) => (
              ...
              >
                ...
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center text-muted-foreground hover:text-red-400 hover:bg-red-400/10 rounded-full transition-colors shrink-0"
                >
        ...
        {addresses.length === 0 && !showForm && (
          <div className="flex flex-col items-center gap-4 py-10 text-center">
            ...
            <button onClick={() => setShowForm(true)} className="px-6 py-3 bg-primary text-white font-bold rounded-full">
              Add First Address
            </button>
          </div>
        )}
        ...
        {showForm && (
          <div className="bg-card border border-border rounded-full p-4 shadow-sm space-y-4">
            ...
            <button
              onClick={locateMe}
              disabled={locating}
              className="w-full h-12 bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-primary/40 rounded-full flex items-center justify-center gap-2 text-primary font-bold text-sm hover:bg-primary/15 transition-colors"
            >
              {locating ? (
            ...
            {/* Map picker */}
            <div className="h-52 rounded-full overflow-hidden border border-border shadow-inner">
              <LocationPickerMap
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {LABELS.map(({ id, icon: Icon, label }) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 p-3 border-2 rounded-full transition-all ${addrLabel === id ? "border-primary bg-primary/8 text-primary" : "border-border text-muted-foreground"}`}
                  >
              ...
              {addrLabel === "Other" && (
                <input
                  ...
                  className="w-full h-11 bg-muted/60 border border-border rounded-full px-3 text-sm mt-2 outline-none focus:border-primary"
                />
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full bg-muted/60 border border-border rounded-full px-3 py-2.5 text-sm outline-none focus:border-primary resize-none"
              />
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 h-12 border border-border rounded-full font-bold text-muted-foreground hover:bg-muted/40"
              >
                Cancel
              </button>
              <button
                onClick={addAddress}
                disabled={addrBusy}
                className="flex-1 h-12 bg-primary text-white font-bold rounded-full disabled:opacity-40 hover:bg-primary/90 transition-colors flex items-center justify-center gap-2"
              >
