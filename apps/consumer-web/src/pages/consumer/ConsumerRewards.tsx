...
export default function ConsumerRewards() {
  ...
  const shareReferral = async () => {
    ...
    try {
      ...
      await Share.share({ title: "FixIt Now Invite", text, url: displayUrl, dialogTitle: "Share your Invite Code" });
    } catch (e) {
      if (navigator.share) {
  ...
  return (
    <ConsumerLayout>
      ...
      <div className="relative z-10 px-4 -mt-8 pb-10 space-y-4">
        {/* Tab selector */}
        <div className="flex bg-card border border-border rounded-full p-1 gap-1 shadow-sm">
        ...
        {activeTab === "referral" && (
          <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={step.n} className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-primary/15 rounded-full flex items-center justify-center shrink-0 text-sm">{step.icon}</div>
            ...
            {/* Referral code */}
            <div className="bg-gradient-to-br from-purple-900/30 to-pink-900/30 border border-purple-500/20 rounded-full p-4">
              <p className="text-xs font-bold text-purple-300 uppercase tracking-wide mb-2">Your Referral Code</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-black/30 border border-white/10 rounded-full px-4 py-3 font-black text-xl tracking-widest text-white text-center">
                  {displayCode}
                </div>
                <button
                  onClick={() => copyCode(displayCode)}
                  className="w-12 h-12 bg-purple-500/20 border border-purple-500/30 rounded-full flex items-center justify-center hover:bg-purple-500/30 transition-colors"
                >
              ...
              <button
                ...
                className="w-full mt-3 h-11 bg-purple-600 text-white font-bold rounded-full flex items-center justify-center gap-2 hover:bg-purple-700 transition-colors"
              >
            ...
            {refStats && (
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={s.label} className="bg-card border border-border rounded-full p-3 text-center">
        ...
        {activeTab === "coupons" && (
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-full p-4 shadow-sm">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 h-11 bg-muted/60 border border-border rounded-full px-3 text-sm font-bold uppercase tracking-widest outline-none focus:border-primary"
                />
                <button
                  onClick={applyCoupon}
                  disabled={applyingCoupon || !couponCode.trim()}
                  className="px-4 h-11 bg-primary text-white font-bold rounded-full disabled:opacity-40 hover:bg-primary/90 transition-colors"
                >
                  Apply
                </button>
              </div>
            ...
            {coupons.map((c: any) => {
              ...
              return (
                <div key={c.coupon_id ?? c.code} className="bg-card border border-dashed border-primary/40 rounded-full p-4 flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="p-2 bg-muted rounded-full hover:bg-muted/60 transition-colors"
                  >
