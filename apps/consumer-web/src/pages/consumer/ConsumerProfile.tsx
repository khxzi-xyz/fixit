...
import {
  ...
  Crown, TrendingUp, Info, Eye, Activity, Map, History
} from "lucide-react";
...
function MenuRow({ icon: Icon, label, sub, href, danger, badge }: { icon: any; label: string; sub?: string; href: string; danger?: boolean; badge?: string }) {
  ...
  return (
    ...
    >
      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${danger ? "bg-red-500/10" : "bg-primary/10"}`}>
  ...
  return (
    <ConsumerLayout>
      ...
      <div className="bg-primary text-primary-foreground border-b border-border px-4 pt-10 pb-20 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <img src="/logo.png" className="w-11 h-11 rounded-full bg-white/20 p-1 shadow-sm" alt="FixIt Now" />
            <span className="text-2xl font-black text-white tracking-tight">FixIt Now</span>
          </div>
          <button onClick={() => navigate("/settings")} className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-colors">
            <Settings className="w-5 h-5 text-white" />
          </button>
        </div>
        <div className="relative z-10 flex items-center gap-4">
          <div className="relative">
            <button onClick={() => navigate("/profile/edit")} className="relative w-16 h-16 rounded-full bg-white/15 border-2 border-white/30 flex items-center justify-center text-white text-2xl font-black shrink-0 shadow-lg overflow-hidden">
              {avatarUrl ? (
            ...
            {plan?.is_lifetime ? (
              <img src="/goldenverifiedbadgepro.png" alt="Lifetime Pro" className="absolute -bottom-2 -right-2 w-8 h-8 drop-shadow-md z-20" />
            ) : hasPlan ? (
              <img src="/bluetickverifiedbadge.png" alt="Plus" className="absolute -bottom-1 -right-1 w-6 h-6 drop-shadow-md z-20" />
            ) : null}
          </div>
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1">
              ...
              <button 
                ...
                className="flex items-center gap-1 px-2 py-0.5 bg-white/10 hover:bg-white/20 transition-colors rounded-full text-white text-[10px] font-bold border border-white/20"
              >
        ...
        {hasPlan && (
          <div className="relative z-10 mt-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-full p-4 shadow-lg">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 bg-amber-400/20 rounded-full flex items-center justify-center shrink-0">
              ...
              {!plan.is_lifetime && (
                <button onClick={() => navigate("/upgrade")}
                  className="px-3 py-2 bg-amber-400 text-black text-xs font-black rounded-full shrink-0 hover:bg-amber-300 transition-colors shadow-sm">
                  + Add days
                </button>
              )}
            </div>
      ...
      <div className="relative z-10 px-4 -mt-10 pb-28 space-y-4">
        ...
        {/* Active Job Tracker */}
        {activeJob && (
          <div className="bg-gradient-to-r from-blue-500/15 via-indigo-500/10 to-blue-500/15 border border-blue-500/30 rounded-full p-4 shadow-sm relative overflow-hidden cursor-pointer" onClick={() => navigate(`/tracking/${activeJob.id}`)}>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button key={label} onClick={() => navigate(href)} className="bg-card border border-border rounded-full p-3 text-center shadow-sm hover:border-primary/40 hover:bg-slate-50 dark:bg-slate-900 transition-all active:scale-95">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 h-12 border border-red-500/30 text-red-400 font-bold rounded-full hover:bg-red-500/10 transition-colors"
        >
