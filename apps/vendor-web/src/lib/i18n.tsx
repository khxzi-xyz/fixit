...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const [dbDict, setDbDict] = useState<Record<string, { en: string; ar: string; ur: string }>>({});

  // Pull the zero-AI translation_dictionary from the backend (no hardcoding).
  useEffect(() => {
    import("@/lib/api").then(({ api }) => api.dictionary().then((d) => setDbDict(d || {})).catch(() => { }));
  }, []);
  ...
  const t = useCallback((key: string, fallback?: string) => {
    // DB dictionary first (en/ar/ur), then the bundled fallback dicts.
    const row = dbDict[key];
    if (row) {
      const v = lang === "ar" ? row.ar : lang === "ur" ? row.ur : row.en;
      if (v) return v;
    }
