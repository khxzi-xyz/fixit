...
export function PromoHero({ title, subtitle, cta, href }: { title: string; subtitle: string; cta: string; href: string }) {
  return (
    <Link href={href}>
      <div className="relative overflow-hidden rounded-2xl p-5 hero-blue text-white shadow-md">
...
export function CategoryTile({ label, Icon, href, color = "primary" }: { label: string; Icon: any; href: string; color?: "primary" | "accent" | "success" | "warning" }) {
  const bg = { primary: "bg-primary/10 text-primary", accent: "bg-accent/10 text-accent", success: "bg-success/10 text-success", warning: "bg-warning/10 text-warning" }[color];
  return (
  ...
  return (
    <Link href={href}>
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
        <div className="relative h-28 hero-blue flex items-center justify-center">
