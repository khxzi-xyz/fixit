        ...
        <div className="absolute z-50 mt-2 w-44 bg-popover border border-border rounded-xl shadow-xl overflow-hidden">
          {CODES.map((c) => (
            <button key={c.code} type="button" onClick={() => { onChange(c.code); setOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted transition-colors ${c.code === value ? "bg-primary/5 text-primary font-bold" : ""}`}>
      ...
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); authMode === "phone" ? submit() : submitEmail(); }}>
        ...
        {authMode === "phone" ? (
          <>
            ...
            <div>
              <label className="block text-sm font-semibold text-foreground mb-1.5">Password (Optional)</label>
              <div className="relative">
              ...
              <p className="text-xs text-muted-foreground mt-1.5">Set a password to skip SMS verification on future logins.</p>
            </div>
          </>
        ) : (
          <>
        ...
        <Button type="submit" disabled={busy} className="w-full h-14 rounded-xl text-base font-bold gap-2 mt-2">
          {busy ? "Loading…" : (authMode === "phone" ? <><span>Send verification code</span><ArrowRight className="w-5 h-5" /></> : <span>Create Account</span>)}
        </Button>
      </form>
