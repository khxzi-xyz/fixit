...
export function TopUpCard({ onCredited }: { onCredited?: () => void }) {
  ...
  const pay = async (method: "paypal" | "dev") => {
    ...
    try {
      ...
      toast({ title: method === "paypal" ? "Paid via PayPal (sandbox)" : "Credited (dev bypass)", description: `+${value} OMR${bonus ? ` and +${bonus} bonus` : ""}` });
      onCredited?.();
  ...
  return (
    <Card className="bg-card border-border shadow-md rounded-2xl overflow-hidden">
      <CardContent className="p-4">
        {/* Bonus ad banner */}
        <div className="hero-blue text-white rounded-2xl p-4 mb-4 relative overflow-hidden">
