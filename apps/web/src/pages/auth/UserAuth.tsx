...
export function UserOTP() {
  ...
  const verify = async (code: string) => {
    ...
    try {
      ...
      navigate(role === "VENDOR" ? "/vendor/home" : "/home");
    } catch (e: any) {
  ...
  return (
    <AuthLayout title="Enter code" subtitle={`Sent to ${phone}`} backTo="/auth/user/login">
      <div className="space-y-6">
          ...
          {digits.map((d, i) => (
            <input
              ...
              className={`w-12 h-14 text-center text-2xl font-black rounded-xl border-2 outline-none transition-all bg-muted/40
                ${d ? "border-primary bg-primary/5 text-primary shadow-[0_0_12px_rgba(27,110,243,0.2)]" : "border-border text-foreground"}
        ...
        <p className="text-center text-xs text-muted-foreground flex items-center justify-center gap-1">
          <MessageSquare className="w-3 h-3" /> OTP delivered via WhatsApp
        </p>
      </div>
    </AuthLayout>
  );
