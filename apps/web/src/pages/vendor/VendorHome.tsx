...
export default function VendorHome() {
  ...
  const load = useCallback(async () => {
    try { const t = await api.bidTokens(); setTokens(t?.tokens ?? 0); } catch { /**/ }
    try { const a = await api.vendorAnalytics(); setEarnings(a?.total_earnings ?? a?.earnings ?? a?.lifetime_earnings ?? 0); } catch { /**/ }
    try { setJobs(await api.vendorMine()); } catch { /**/ }
  }, []);
  useEffect(() => { load(); }, [load]);
    ...
    <VendorLayout>
      ...
      <div className="px-4 -mt-6 space-y-6">
        {active && (
          <div>
            ...
            <Link href={`/vendor/order/${active.job_id}`}>
              <Card className="bg-card border-border shadow-md rounded-2xl relative overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <Link href="/vendor/jobs">
          <div className="bg-card border border-border rounded-2xl shadow-sm p-4 flex items-center justify-between hover:shadow-md transition-shadow">
            <div>
