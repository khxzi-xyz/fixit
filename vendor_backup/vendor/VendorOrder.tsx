...
export default function VendorOrder() {
  ...
  // Stream GPS pings while en route; poll for the consumer's destination.
  const startPinging = useCallback(() => {
    if (!jobId || watchId.current != null || !navigator.geolocation) return;
    watchId.current = navigator.geolocation.watchPosition(
      (p) => { const loc = { lat: p.coords.latitude, lng: p.coords.longitude }; setMyPos(loc); api.trackingPing(jobId, loc.lat, loc.lng).catch(() => {}); },
      () => {}, { enableHighAccuracy: true, maximumAge: 4000, timeout: 8000 },
    );
  }, [jobId]);
  const stopPinging = useCallback(() => { if (watchId.current != null) { navigator.geolocation.clearWatch(watchId.current); watchId.current = null; } }, []);

  useEffect(() => {
    if (status === "EN_ROUTE") {
      startPinging();
      const id = setInterval(() => { if (jobId) api.trackingSession(jobId).then((s) => { if (s?.destination) setDest(s.destination); }).catch(() => {}); }, 6000);
  ...
  const onWay = async () => { if (!jobId) return; setBusy(true); try { await api.startTracking(jobId); setStatus("EN_ROUTE"); startPinging(); toast({ title: "Client notified you're on the way" }); } catch (e) { toast({ title: "Error", description: e instanceof Error ? e.message : String(e) }); } finally { setBusy(false); } };
  const arrived = async () => { if (!jobId) return; setBusy(true); try { await api.arriveTracking(jobId); setStatus("ARRIVED"); stopPinging(); toast({ title: "Marked as arrived" }); } catch (e) { toast({ title: "Error", description: e instanceof Error ? e.message : String(e) }); } finally { setBusy(false); } };

  const capture = async (phase: Phase, file?: File) => {
  ...
  const PhotoTile = ({ phase, label, canUpload = false }: { phase: Phase; label: string; canUpload?: boolean }) => {
    ...
    return (
      <div className="aspect-square bg-muted rounded-xl border border-dashed border-border flex flex-col items-center justify-center overflow-hidden relative">
        {canUpload && (
          <input ref={fileRefs[phase as "BEFORE"|"VENDOR_AFTER"]} type="file" accept="image/*" className="hidden" onChange={(e) => capture(phase, e.target.files?.[0])} />
        )}
        {isUploaded ? (
          <img src={photos[phase]} alt={label} className="w-full h-full object-cover" />
        ) : canUpload ? (
          <button onClick={() => fileRefs[phase as "BEFORE"|"VENDOR_AFTER"].current?.click()} disabled={busy} className="flex flex-col items-center justify-center w-full h-full hover:bg-muted/80">
      ...
      <div className="px-4 -mt-6 space-y-6 pb-6">
        <Card className="bg-card border-border shadow-md rounded-2xl">
          <CardContent className="p-4">
            {status === "ASSIGNED" && (
              <Button onClick={onWay} disabled={busy} className="w-full h-14 rounded-xl text-lg font-bold">
            ...
            {status === "EN_ROUTE" && (
              <div className="space-y-3">
                <div className="flex gap-2">
                  <Button variant="outline" className="flex-1 h-12 rounded-xl border-border"><Phone className="w-4 h-4 mr-2" /> Call</Button>
                  <Button variant="outline" className="flex-1 h-12 rounded-xl border-border"><MapPin className="w-4 h-4 mr-2" /> Map</Button>
                </div>
                <Button onClick={arrived} disabled={busy} className="w-full h-14 rounded-xl text-lg font-bold">I Have Arrived</Button>
              </div>
            )}
            {working && (
        ...
        {status === "EN_ROUTE" && (
          <Card className="bg-card border-border shadow-md rounded-2xl overflow-hidden">
            <div className="h-56 relative">
              ...
              <div className="absolute bottom-2 left-2 right-2 bg-background/90 backdrop-blur rounded-xl px-3 py-2 text-xs font-semibold flex items-center gap-2">
        ...
        <div className={`space-y-4 ${!working ? "opacity-50 pointer-events-none" : ""}`}>
          ...
          <Button disabled={!canRequest || busy} onClick={() => toast({ title: "Sent for client approval", description: "The client will review your photos and release funds." })}
            className="w-full h-14 rounded-xl text-lg font-bold bg-success hover:bg-success/90 text-success-foreground">
